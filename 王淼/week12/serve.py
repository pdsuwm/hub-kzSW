...
# ── 会话存储（内存字典，session_id -> conversation_history）──────────────────────
...
async def _stream_react(question: str, max_steps: int, mode: str, session_id: str = None):
    ...
    logger.info(f"处理请求: session_id={session_id}, history_len={len(history) if history else 0}, question={question[:50]}...")
    ...
    while True:
        ...
        if step_data.get("type") == "final" and step_data.get("history") and session_id:
            _save_session_history(session_id, step_data["history"])
