...
使用方式：
  ...
  python src/probe_baseline.py --model outputs/grpo_ckpt  # 评估训练后的模型

输出：
  outputs/baseline_probe.json              # 各难度指标 + 样例输出

环境变量：
  MODEL_PATH  指定本地模型路径，未设置时默认从 HuggingFace 下载 Qwen/Qwen2-0.5B-Instruct
"""
import os
...
# 云 GPU 适配：优先用环境变量 MODEL_PATH，否则默认从 HuggingFace 下载
MODEL_PATH = os.environ.get("MODEL_PATH", "Qwen/Qwen2-0.5B-Instruct")
