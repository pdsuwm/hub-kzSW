...
规律：x是一个 input_size 维向量，哪一维数字最大就属于第几类。
"""
...
def evaluate(model):
    model.eval()
    test_sample_num = 100
    x, y = build_dataset(test_sample_num, model.input_size)
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测 model.forward(x)
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            # print(f"预测值: {y_p} | 真实值: {y_t}")
...
if __name__ == "__main__":
    ...
    #             [0.90797868,0.67482528,0.93625847,0.34675372,0.19871392],
    #             [0.39349776,0.59416669,0.42579291,0.41567412,0.2358894]]
    # predict("model.pt", test_vec, 5)


'''
激活函数	       输出范围	 多分类适用性   说明
无（raw logits）   (-∞, +∞)	 ✅ 推荐      让 cross_entropy 自行处理
ReLU	          [0, +∞)   ✅ 可用      保持正值，适合多分类
Tanh	          [-1, 1]   ⚠️ 不理想    范围太窄，限制表达能力  
Sigmoid	          (0, 1)    ❌ 不适合    用于二分类/多标签
'''